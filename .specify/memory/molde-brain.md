...
> Apps decommissioned 2026-08-09 (`paramalhar` and `recibos`; local folders deleted 2026-08-15). `recibos` DB (`f13lkyius8n7ctiyksxfhx6u`, `10.0.2.4`) is gone. ⚠️ `paramalhar` DB (`postgresql-database-py93j9ymwzqdszeq5p2qvxdu`, `pgvector/pgvector:pg18`, DB `paramalhar` / user `paramalhar_app`, ~11 MB) **survived**: the 2026-08-09 `DELETE /api/v1/databases/{uuid}` answered `deletion request queued` and never executed — found 2026-08-15 still `running:healthy`, backing up daily to R2, and absent from `GET /api/v1/databases` (only `GET /api/v1/databases/py93…` shows it). No app referenced it. Deleted for good by Gustavo on 2026-08-15 (Coolify UI → Danger Zone, delete volumes + docker cleanup) — verified: `GET /api/v1/databases/py93…` → 404, `docker ps -a` and `docker volume ls` on the VPS both empty for `py93`. Lesson: after an async delete, poll `GET /databases/{uuid}` until 404 — the list endpoint can hide it. See field-notes `[2026-08-09] recibos + paramalhar — infra: deprovision…` and `[2026-08-15] recibos + paramalhar + mercado + taskly — deprovision, round 2…`.
